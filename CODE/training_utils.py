...
def get_optimizer(cfg: ProjectConfig, model: torch.nn.Module, accelerator: Accelerator) -> torch.optim.Optimizer:
    ...
    print("In train utils: get_optimizer()cfg model accelerator")
...
def get_scheduler(cfg: ProjectConfig, optimizer: torch.optim.Optimizer) -> Callable:
    ...
    # Get scheduler
    print("In train utils: get_scheduler()cfg optimizer")
...
def accuracy(output, target, topk=(1,)):
    ...
    print("In training utils : accuracy()output target topk")
    maxk = max(topk)
...
class SmoothedValue(object):
    ...
    def update(self, value, n=1):
        print("In training utils : update()self value n")
    ...
    def synchronize_between_processes(self, device='cuda'):
        ...
        """
        print("In training utils : ynchronize_between_processes()self device")
    ...
    @property
    def median(self):
        print("In training utils : median()self")
        d = torch.tensor(list(self.deque))
        return d.median().item()

    @property
    def avg(self):
        print("In training utils : avg()self")
        d = torch.tensor(list(self.deque), dtype=torch.float32)
        return d.mean().item()

    @property
    def global_avg(self):
        print("In training utils : lobal_avg()self")
        return self.total / max(self.count, 1)

    @property
    def max(self):
        print("In training utils : max()self")
        return max(self.deque)

    @property
    def value(self):
        print("In training utils : value()self")
        return self.deque[-1]
...
class MetricLogger(object):
    ...
    def update(self, **kwargs):
        print("In training utils : update()self **kwargs")
        n = kwargs.pop('n', 1)
    ...
    def __getattr__(self, attr):
        print("In train utils: __getattr__()self attr")
    ...
    def __str__(self):
        print("In train utils: __str__()self")
        loss_str = []
    ...
    def synchronize_between_processes(self, device='cuda'):
        print("In training utils : synchronize_between_processes()self device")
        for meter in self.meters.values():
            meter.synchronize_between_processes(device=device)

    def add_meter(self, name, meter):
        print("In training utils : add_meter()self name meter")
        self.meters[name] = meter

    def log_every(self, iterable, print_freq, header=None):
        print("In training utils : log_every()self iterable print_freq header")
        i = 0
        if not header:
            header = ''
...
class NormalizeInverse(torchvision.transforms.Normalize):
    ...
    def __call__(self, tensor):
        print("In train utils: __call__()self tensor")
        return super().__call__(tensor.clone())


def resume_from_checkpoint(cfg: ProjectConfig, model, optimizer=None, scheduler=None, model_ema=None):
    print("In training utils : resume_from_checkpoint()cfg model optimizer scheduler model_ema")
...
def using_distributed():
    print("In training utils : using_distributed()")
    return dist.is_available() and dist.is_initialized()


def get_rank():
    print("In training utils : get_rank()")
    return dist.get_rank() if using_distributed() else 0


def set_seed(seed):
    print("In training utils : set_seed()seed")
    rank = get_rank()
    seed = seed + rank
...
def compute_grad_norm(parameters):
    ...
    print("In training utils : compute_grad_norm()parameters")
    total_norm = 0
    for p in parameters:
