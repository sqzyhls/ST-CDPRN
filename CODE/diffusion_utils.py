...
def rotation_matrix(axis, theta):
    ...
    """
    print("In diffusion utils : rotation matrix()axis theta")
...
def rotate(vertices, faces):
    ...
    '''
    print("In diffusion utils : rotate()vertices faces")
...
def norm(v, f):
    print("In diffusion utils : norm()v f")
...
def getGradNorm(net):
    print("In diffusion utils : getGrandNorm()net")
...
def weights_init(m):
    print("In diffusion utils : weights_init()m")
...
def discretized_gaussian_log_likelihood(x, *, means, log_scales):
    print("In diffusion utils : discretized_gaussian_log_likelihood()x * means log_scales")
...
def fig2img(fig):
    ...
    print("In diffusion utils : fig2img()fig")
    import io
    buf = io.BytesIO()
    fig.savefig(buf)
    buf.seek(0)
...
def visualize_distance_transform(
    ...
    print("In diffusion utils : visualize_distance_transform()path_stem images")
...
def visualize_image(
    ...
    print("In diffusion utils : visualize_image()path_stem images mean std")
    output_file_image = f'{path_stem}.png'
...
def ensure_point_cloud_has_colors(pointcloud: Pointclouds):
    print("In diffusion utils : ensure point cloud has colors()Pointcloud")
...
def render_pointcloud_batch_pytorch3d(
    ...
    print("In diffusion utils : render_pointcloud_batch_pytorch3d()cameras pointclouds image_size radius points_per_pixel background_color compositor")
...
def visualize_pointcloud_batch_pytorch3d(
    ...
    print("In diffusion utils : visualize_pointcloud_batch_pytorch3d()pointclouds output_file_video output_file_image cameras scale_factor num_frames elev")
...
def visualize_pointcloud_evolution_pytorch3d(
    ...
    print("In diffusion utils : visualize_pointcloud_evolution_pytorch3d()pointclouds output_file_videon camera scale_factor")
    # Device
...
def get_camera_index(cameras: CamerasBase, index: Optional[int] = None):
    print("In diffusion utils : get_camera_index()cameras index")
    if index is None:
        return cameras
    kwargs = dict(
...
def get_metadata(item) -> str:
    print("In diffusion utils : get_metadata()item")
    s = '-------------\n'
