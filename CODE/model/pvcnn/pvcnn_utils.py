...
def _linear_gn_relu(in_channels, out_channels):
    print("In pvcnn utils: _linear_gn_relu()in_channels out_channels")
    return nn.Sequential(nn.Linear(in_channels, out_channels), nn.GroupNorm(8,out_channels), Swish())


def create_mlp_components(in_channels, out_channels, classifier=False, dim=2, width_multiplier=1):
    print("In pvcnn utils: create_mlp_components()in_channels out_channels classifier dim width_multiplier")
    r = width_multiplier
...
def create_pointnet_components(blocks, in_channels, embed_dim, with_se=False, normalize=True, eps=0,
    ...
    print("In pvcnn utils: create_pointnet_components()blocks in_channels embed_dim with_se normalize eps width_multiplier voxel_resolution_multiplier")
...
def create_pointnet2_sa_components(sa_blocks, extra_feature_channels, embed_dim=64, use_att=False,
                                   ...
                                   width_multiplier=1, voxel_resolution_multiplier=1):
    print("In pvcnn utils: create_pointnet2_sa_components()a_blocks extra_feature_channels embed_dim use_att dropout with_se normalize eps width_multiplier voxel_resolution_multiplier")
    r, vr = width_multiplier, voxel_resolution_multiplier
...
def create_pointnet2_fp_modules(fp_blocks, in_channels, sa_in_channels, embed_dim=64, use_att=False,
                                ...
                                width_multiplier=1, voxel_resolution_multiplier=1):
    print("In pvcnn utils: create_pointnet2_fp_modules()fp_blocks in_channels sa_in_channels embed_dim=64 use_att dropout with_senormalize eps width_multiplier voxel_resolution_multiplier")
    r, vr = width_multiplier, voxel_resolution_multiplier
...
def get_timestep_embedding(embed_dim, timesteps, device):
    ...
    """
    print("In pvcnn utils: get_timestep_embedding()embed_dim timesteps device")
