...
class PointCloudProjectionModel(ModelMixin):
    ...
    def get_global_conditioning(self, image_rgb: Tensor):
        ...
        print("global_conditioning shape:",global_conditioning.shape)
        return global_conditioning

    def get_local_conditioning(self, image_rgb: Tensor):
        print("In projection model: get_local_conditioning()self image_rgb mask")
        local_conditioning = []
        if self.use_local_colors:
            local_conditioning.append(self.normalize(image_rgb))
        if self.use_local_features:
            local_conditioning.append(self.feature_model(image_rgb))
        print("local_conditioning:",local_conditioning)
        local_conditioning = torch.cat(local_conditioning, dim=1)  # (B, D_cond, H, W)
        print("local_conditioning shape:",local_conditioning.shape)
    ...
    def surface_projection(
        ...
        print("In projection model: surface_projection()self points camera local_features")
        ...
        print("local_features_proj shape:",local_features_proj.shape)
    ...
    def point_cloud_to_tensor(self, pc: Pointclouds, /, normalize: bool = False, scale: bool = False):
        ...
        print("In projection model: point_cloud_to_tensor()self pc normalize scale")
    ...
    def tensor_to_point_cloud(self, x: Tensor, /, denormalize: bool = False, unscale: bool = False):
        print("In projection model: tensor_to_point_cloud()self x / denormalize unscale")
    ...
    def get_input_with_conditioning(
        ...
        print("In projection model: get_input_with_conditioning()self x_t clamp image_rgb mask t")
        B, N = x_t.shape[:2]
        ...
        if self.use_local_conditioning:
            ...
            local_features = self.get_local_conditioning(image_rgb=image_rgb)
    ...
    def forward(self, batch: FrameData, mode: str = 'train', **kwargs):
        ...
        print("In projection model: forward()self batch mode **kwargs")
