...
class ConditionalPointCloudDiffusionModel(PointCloudProjectionModel):
    ...
    def forward_train(
        ...
        print("In model : forward_train()self pc camera image_rgb mask return_intermediate_steps")
        # Normalize colors and convert to tensor
        x_0 = self.point_cloud_to_tensor(pc, normalize=True, scale=True)
        B, N, D = x_0.shape
        print("Input Pointclouds Shape:",x_0.shape)
        # Sample random noise
        ...
        x_t_input = self.get_input_with_conditioning(x_t, camera=camera, 
            image_rgb=image_rgb,  t=timestep)
        print("Conditional Input shape：",x_t_input.shape)
        # Forward
    ...
    def forward_sample(
        ...
        print("In model : forward_sample()self num_points camera image_rgb mask scheduler num_inference_steps eta return_sample_every_n_steps disable_tqdm")
    ...
    def forward(self, batch: FrameData, mode: str = 'train', **kwargs):
        ...
        print("In model : forward()self batch mode **kwargs")
        ...
        if mode == 'train':
            print("model177--------------------------------")
            print(batch)
            print(batch.sequence_point_cloud)
