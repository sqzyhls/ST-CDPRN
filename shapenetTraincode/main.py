...
def main(cfg: ProjectConfig):
    ...
    while True:
        ...
        progress_bar: Iterable[Any] = metric_logger.log_every(dataloader_train, cfg.run.print_step_freq, 
            header=log_header)
        if train_state.epoch % 1 == 0:
            all_cham_dist = []
            all_f1 = []
        # Train
        for i, batch in enumerate(progress_bar):
            #print(i)
            #model(batch, mode='train', returnl_eval_score=True)
            #continue
            ...
            with accelerator.accumulate(model):
                ...
                # Forward
                if train_state.epoch % 1 == 0:
                    loss, (cham_dist, f1) = model(batch, mode='train', returnl_eval_score=True)
                    all_cham_dist.append(cham_dist.cpu())
            ...
            if accelerator.sync_gradients:
                ...
                # Save a checkpoint
                #print(accelerator.is_main_process,train_state.step,cfg.run.checkpoint_freq)
                ...
                #break
        if train_state.epoch % 1 == 0:
            cham_dist = torch.cat(all_cham_dist,dim=0)
            cham_dist = cham_dist.mean()
            f1 = torch.cat(all_f1,dim=0)
            f1 = f1.mean()
            print('cham: ', cham_dist)
            print('f1 score: ', f1)
...
def visualize(
    *,
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        torch.save({'output': output, 'all_outputs': all_outputs, 'batch': trans_batch}, filename)

        # Save metadata
        metadata = diffusion_utils.get_metadata(trans_batch)
        filename = filestr.format(dir='metadata', name='metadata', i=0, ext='txt')
        Path(filename).write_text(metadata)

        # Save individual samples
        for i in range(len(output)):
            camera = trans_batch.camera[i]
            gt_pointcloud = trans_batch.sequence_point_cloud[i]
            ...
            TVF.to_pil_image(trans_batch.image_rgb[i]).save(filename)
    ...
    *,
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        for sample_idx in range(cfg.run.num_samples):
            ...
            print(trans_batch.sequence_category[0])
            # Save individual samples
            for i in range(len(output)):
                
                sequence_name = trans_batch.sequence_name[i]
                sequence_category = trans_batch.sequence_category[i]
                ...
                # Save ground truth
                io.save_pointcloud(data=trans_batch.sequence_point_cloud[i], path=filestr.format(dir='gt', 
                ...
                metadata = dict(index=i, sequence_name=trans_batch.sequence_name, sequence_category=trans_batch.sequence_category,
                                frame_timestamp=trans_batch.frame_timestamp, camera=trans_batch.camera, image_size_hw=trans_batch.image_size_hw,
                                image_path=trans_batch.image_path, depth_path=trans_batch.depth_path, mask_path=trans_batch.mask_path,
                                bbox_xywh=trans_batch.bbox_xywh, crop_bbox_xywh=trans_batch.crop_bbox_xywh,
                                sequence_point_cloud_path=trans_batch.sequence_point_cloud_path, meta=trans_batch.meta)
