...
import time
import torch
from .utils import chamfer_distance, fscore

from config.structured import CO3DConfig, DataloaderConfig, ProjectConfig, ShapeNetR2N2Config
class ConditionalPointCloudDiffusionModel(PointCloudProjectionModel):
    ...
    def forward_train(
        ...
        if returnl_eval_score:
            cham_dist, cham_normals, cham_x, cham_y = chamfer_distance(noise_pred, noise, batch_reduction=None, point_reduction=None)
            f1, precision_1, precision_2 = fscore(cham_x, cham_y)
            return loss, (cham_dist.mean(1), f1)
    ...
    def trans_batch(self,batch):
        ori_keys = list(batch.keys())
        #new_batch = {}
        batch['sequence_point_cloud'] = Pointclouds(batch['verts'])
        # 下采样
        batch['sequence_point_cloud'] = batch['sequence_point_cloud'].subsample(max_points=ShapeNetR2N2Config.max_points) 
        #print(batch['sequence_point_cloud']._num_points_per_cloud)
        #del batch['verts']
        batch['synset_id'] = batch['model_id'] 
        batch['frame_number'] = batch['model_id'] 
        batch['sequence_name'] = batch['model_id'] 
        batch['sequence_category'] = batch['label'] 
        #print(len(new_batch['sequence_point_cloud']))
        batch['camera'] = PerspectiveCameras(K=batch['K'][:,0],T=batch['T'][:,0],R=batch['R'][:,0],device='cuda')
        #!!!取一张图像
        #resize to 224
        #new_batch['image_rgb'] = batch['images'][:,0,:,:,:].permute(0, 3, 1, 2)
        batch['image_rgb'] = F.interpolate(batch['images'][:,0,:,:,:].permute(0, 3, 1, 2), size=(224, 224), mode='bicubic', align_corners=False)
        #print(batch['image_rgb'].shape)
        #print(batch['images'].shape)
        batch['fg_probability'] = torch.ones(batch['image_rgb'].shape[0],1,224,224).to('cuda')
        list(map(batch.pop,ori_keys))
        return batch